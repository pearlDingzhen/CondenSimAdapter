...
from CondenSimAdapter.extern.ms2_openabc.lib import NA, kB, EC, VEP, _amino_acids, _dna_nucleotides
import math
import sys
import os
...
def elastic_network_term(enm_pairs, use_pbc, force_constant=700.0*unit.kilojoule_per_mole/unit.nanometer**2, force_group=4):
    """
    Elastic Network Model (ENM) term for maintaining folded domain structure.
    
    This function creates harmonic restraints between residue pairs within folded domains,
    using the reference distances from the input structure.
    
    Parameters
    ----------
    enm_pairs : list of tuples
        List of (i, j, r0) tuples where:
        - i: global index of first residue (0-based)
        - j: global index of second residue (0-based)
        - r0: reference distance in nm
    ...
    force : mm.CustomBondForce
        OpenMM force object for elastic network.
    
    """
    k_value = force_constant.value_in_unit(unit.kilojoule_per_mole/unit.nanometer**2)
    
    # Create harmonic bond force for elastic network
    elastic_force = mm.CustomBondForce("0.5*k*(r-r0)^2")
    elastic_force.addGlobalParameter("k", k_value)
    elastic_force.addPerBondParameter("r0")
    
    # Add all pairs
    for i, j, r0 in enm_pairs:
        elastic_force.addBond(i, j, [r0])
    
    # Set periodic boundary conditions
    elastic_force.setUsesPeriodicBoundaryConditions(use_pbc)
    elastic_force.setForceGroup(force_group)
    
    return elastic_force


def elastic_network_go_term(enm_pairs, use_pbc, force_constant=1.0, force_group=4):
    """
    Go-like potential for elastic network.
    
    This uses a Go-like potential that is attractive only at the reference distance
    and neutral otherwise, useful for maintaining native-like contacts.
    ...
    -------
    force : mm.CustomBondForce
        OpenMM force object for Go-like elastic network.
    
    """
    # Go-like potential: attractive at native distance, neutral otherwise
    # Using the form: k * (5*(s/r)^12 - 6*(s/r)^10)
    go_expr = 'k*(5*(s/r)^12-6*(s/r)^10)'
    go_force = mm.CustomBondForce(go_expr)
    go_force.addGlobalParameter("k", force_constant)
    go_force.addPerBondParameter("s")
    
    # Add all pairs with reference distance as sigma
    for i, j, r0 in enm_pairs:
        go_force.addBond(i, j, [r0])
    
    # Set periodic boundary conditions
    go_force.setUsesPeriodicBoundaryConditions(use_pbc)
    go_force.setForceGroup(force_group)
