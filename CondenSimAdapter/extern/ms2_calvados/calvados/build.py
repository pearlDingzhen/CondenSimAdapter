...
def geometry_from_pdb(pdb,use_com=False):
    """ positions in nm
    If PDB has no CRYST1 line, auto-compute box from atom coordinates.
    """
    with catch_warnings():
        simplefilter("ignore")
        u = Universe(pdb)
    ...
    # Handle PDB without CRYST1 line
    if u.dimensions is None:
        # Auto-compute box from atom coordinates (with 2nm padding)
        box_size = np.max(pos, axis=0) - np.min(pos, axis=0) + 2.0  # nm
        box = np.append(box_size, [90.0, 90.0, 90.0])
    else:
